...
public class KeyPair {
	...
	// функци€ провер€ет €вл€ютс€ ли числа взаимопростыми
		public static boolean isCoprimeNumber(BigInteger a, BigInteger b) {
			...
			for (BigInteger i = new BigInteger("2"); i.compareTo(a.min(b)) <= 0; i=i.add(one)) {
				// если числа имеют общие делители, функци€ возвращает ложь
				if ((a.mod(i).compareTo(zero) == 0) && (b.mod(i).compareTo(zero) == 0)) {
					return false;
				}
			}
			// функци€ возвращает истину
			return true;	
		}
	...
	public static KeyPair genKeyPair() {
				...
				// нахождение значени€ функции Ёйлера
				BigInteger one = new BigInteger("-1");
...
// переопределенна€ функци€ toString
	@Override
